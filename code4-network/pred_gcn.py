...
import sys
import os
sys.path.append( f'{os.path.dirname(os.path.realpath(__file__))}/structure')
...
from utils import *
from structure.models import GCN_DEEP_DIVER
import time
import argparse
import pathlib
N_bd = 57
...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('model', 'gcn_cheby', 'Model string.')  # 'structure', 'gcn_cheby', 'dense'
...
def evaluate(features, support, placeholders, model):
    t_test = time.time()
...
def predict(modelpath):
    ...
    placeholders = {
        ...
        if FLAGS.matrix_type == 'sparse' else tf.placeholder(tf.float32, shape=(None, feat_dim)),
        # featureless: #points
    ...
    model_dir = f'{filedir}/saved-models/ds/'+modelpath

    data_path = f'{filedir}/datasets/ds/test'
    # data_files = [f'{data_path}/sample_{i}.pkl' for i in range(999)]
    ...
    for data_file in data_files:
        ...
        if FLAGS.matrix_type == 'sparse':
            ...
            support = simple_polynomials(adj, FLAGS.max_degree) if FLAGS.model == 'gcn_cheby' else [
                preprocess_adj_sparse(adj)]
        else:
            support = simple_polynomials_to_dense(adj, FLAGS.max_degree) if FLAGS.model == 'gcn_cheby' else [
                preprocess_adj(adj)]

        _, z_out = evaluate(xs, support, placeholders,model)
...
if __name__ == '__main__':
   predict('GG-GCN_20250103_161517')
