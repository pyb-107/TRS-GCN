...
class GCN_DEEP_DIVER(Model):
    ...
    def _loss(self):
        ...
        # 32 outputs
        diver_loss = masked_softmax_cross_entropy(self.outputs[:,0:self.output_dim], self.placeholders['labels'], self.placeholders['labels_mask'])
        for i in range(1,FLAGS.diver_num):
            diver_loss = tf.reduce_min([diver_loss, masked_softmax_cross_entropy(self.outputs[:, 2*i:2*i + self.output_dim], self.placeholders['labels'], self.placeholders['labels_mask'])])
    ...
    def _accuracy(self):
        # 32 outputs
        acc = masked_accuracy(self.outputs[:,0:self.output_dim], self.placeholders['labels'], self.placeholders['labels_mask'])
        # acc = my_accuracy(self.outputs[:,0:self.output_dim], self.placeholders['labels'])
        for i in range(1,FLAGS.diver_num):
            acc = tf.reduce_max([acc, masked_accuracy(self.outputs[:,2*i:2*i+self.output_dim], self.placeholders['labels'], self.placeholders['labels_mask'])])
        self.accuracy = acc
