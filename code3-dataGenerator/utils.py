import numpy as np
import PySCIPOpt.src.pyscipopt as scip
...
def extract_ding_variable_features(model):
    ...
    # 最终我们会得到两个关键的特征矩阵，colState表示变量特征，row表示问题约束的特征
    # vc是一个二维数组，用于存储约束与变量之间的系数关系。它的维度是(nrows, ncols)，表示每个约束与每个变量之间的系数。
    # vo是一个一维数组，用于存储每个变量的目标函数系数。
    # co是一个一维数组，用于存储每个约束的右端值（RHS）。
    # 也就是说将一整个lp问题转化成了三个数组
